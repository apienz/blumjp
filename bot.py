...
def print_welcome_message():
    print(r"""
           CUAN CUAN CUAN CUAN CUAN CUAN CUAN CUAN
          """)
...
while True:
    ...
    for query_id in query_ids:
        ...
        while balance_info['playPasses'] > 0:
            ...
            time.sleep(1)
            claim_response = claim_game(token, game_response['gameId'], random.randint(180,200))
            ...
            while True:
                if claim_response.text == '{"message":"game session not finished"}':
                    ...
                    print(f"\r{random_color+Style.BRIGHT}[ Play Game ] : %%%%%%%%% LOADING %%%%%%%%%%%", flush=True)
                    claim_response = claim_game(token, game_response['gameId'], random.randint(180,200))
                    if claim_response is None:
                        print(f"\r{Fore.RED+Style.BRIGHT}[ Play Game ] : Gagal mengklaim game, mencoba lagi...", flush=True)
            ...
            if balance_info is None: # Refresh informasi saldo untuk mendapatkan tiket terbaru
                ...
                if balance_info['playPasses'] > 0:
                    print(f"\r{Fore.GREEN+Style.BRIGHT}[ Play Game ] : sisa tiket {balance_info['playPasses']}", flush=True)
    ...
    import sys
    waktu_tunggu = 14400  # waktu looping dalam detik
    for detik in range(waktu_tunggu, 0, -1):
        sys.stdout.write(f"\r{Fore.CYAN} WAKTU TUNGGU BERIKUTNYA {Fore.CYAN}{Fore.WHITE}{detik // 60 } menit {Fore.WHITE}{detik % 60} detik")
        sys.stdout.flush()
        time.sleep(1)
